...
if [[ $(ps -edf | grep -c "launch-ezp.sh") = 4 ]];then
  ...
  # Liste complète des colonnes définies lors du projet EZStats (2024) : les données de certaines colonnes ne sont pour l'instant pas renvoyées par l'API Theses.fr. Ces colonnes ne sont donc pas utilisées (ex : doiThese)
